...
NearNeighborDisMetric::NearNeighborDisMetric(NearNeighborDisMetric::METRIC_TYPE metric_type, float matching_threshold, int budget)
{
  if(metric_type == euclidean){
    func_ptr = &NearNeighborDisMetric::_nneuclidean_distance;
  } 
  else if (metric_type == cosine){
    func_ptr = &NearNeighborDisMetric::_nncosine_distance;
  }
  else{
        ...
        if(newSize <= this->budget) {
            ...
            if(oldSize < this->budget) {//original space is not enough;
                ...
                if(addSize >= this->budget) {
                    ...
                    newSampleFeatures.block(0, 0, this->budget-addSize, feature_dims) = samples[track_id].block(addSize-1, 0, this->budget-addSize, feature_dims).eval();
                ...
                if(addSize >= this->budget) {
                    ...
                    samples[track_id].block(0, 0, this->budget-addSize, feature_dims) = samples[track_id].block(addSize-1, 0, this->budget-addSize, feature_dims).eval();
...
{
  // 针对trk的每个特征（每个特征都有一个特征集targets），计算它们与检测特征features之间的（1-余弦距离）。然后取最小值作为该trk与检测结果之间的计算值
  DYNAMICM cost_matrix = Eigen::MatrixXf::Zero(targets.size(), features.rows());
  int idx = 0;
  for(int target:targets) {
    cost_matrix.row(idx) = (this->*func_ptr)(this->samples[target], features);
    idx++;
  }
  return cost_matrix;
}
